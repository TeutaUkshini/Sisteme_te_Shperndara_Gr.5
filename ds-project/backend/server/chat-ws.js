...
const debugClient = require('debug')('chat-ws:client')
const debugTopic = require('debug')('chat-ws:topic')
const WebSocketNode = require('ws')
const { Server } = WebSocketNode
const CONN_TIMEOUT = 15000


class ChatWS extends Server {
  /**
  ...
  _createTopic (uid, topic) {
    ...
    debugTopic('client %s created topic %s', uid, topic)
  }
